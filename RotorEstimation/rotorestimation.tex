...
\end{array}\right]$}
\STATE{$M_{00} = S^T S + Tr(B)$}
\STATE{$M_{11} = 2 B_{00} + S^T S - Tr(B)$}
\STATE{$M_{22} = 2 B_{11} + S^T S - Tr(B)$}
\STATE{$M_{33} = 2 B_{22} + S^T S - Tr(B)$}
\FOR{$i = 1$ \TO $12$}
\STATE{$M^{i+1} = \frac{M^i M^i}{ Tr(M^i) }$}
\ENDFOR
\STATE{$\mathcal C_j = M_{j0}, \forall j = 0, ..., 3 $}
...
\end{algorithm}
The C++ code using the Eigen library \cite{Eigen} can be found in Listing~\ref{lst:cppcode}.
    ...
    const vector<double>& w)
{
    ...
    M(3, 1) = (B(2, 0) - B(0, 2));
    M(3, 2) = (B(0, 1) - B(1, 0));
    M(0, 0) = 2.0 * B(0, 0) + diag;
    M(1, 0) = (B(0, 1) + B(1, 0)); 
    M(2, 0) = (B(2, 0) + B(0, 2));
    M(1, 1) = 2.0 * B(1, 1) + diag; 
    M(2, 1) = (B(1, 2) + B(2, 1));
    M(2, 2) = 2.0 * B(2, 2) + diag;
    M.selfadjointView<Eigen::Lower>().evalTo(H);
    for (size_t j = 0; j < MAX_STEPS; ++j) {
        M *= M;
        M *= 1.0 / M.trace();
    }
    return Quaterniond(M.col(0).normalized());
}
\end{lstlisting}
